...
void bench_echo_one_byte()
{
    ...
    auto server_future = fibers::async([&server, &num_clients, &num_iterations]() {
        for (int c = 0; c < num_clients; c++)
        {
            auto server_client = server.accept();
            fibers::async([&num_iterations](fiberio::socket client) {
                char buf[1];
    ...
    {
        futures.at(i) = fibers::async([&num_iterations](fiberio::socket client) {
            char buf[]{'a'};
            char buf2[1];
            for (uint64_t i = 0; i < num_iterations; i++)
            {
