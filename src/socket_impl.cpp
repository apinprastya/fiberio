...
namespace fiberio
{
    ...
    namespace
    {
        ...
        void alloc_callback(uv_handle_t *handle, size_t suggested_size, uv_buf_t *buf)
        {
            ...
            if (DEBUG_LOG)
                std::cout << "using buffer from socket of " << socket->get_len() << " bytes\n";
    ...
    socket_impl::~socket_impl()
    {
        ...
        {
            if (DEBUG_LOG)
                std::cout << "closing socket_impl from destructor\n";
            close();
        }
        else
        {
            if (DEBUG_LOG)
                std::cout << "destroying closed socket_impl\n";
        }
    }
    ...
    std::size_t socket_impl::read(char *buf, std::size_t size)
    {
        ...
        {
            if (DEBUG_LOG)
                std::cout << "socket_impl: concurrent read\n";
...
// we const_cast since the API incorrectly takes a mutable char* buffer
#ifdef __linux__
        const uv_buf_t bufs[]{{.base = const_cast<char *>(data),
                               .len = len}};
#else
        const uv_buf_t bufs[]{{len, const_cast<char *>(data)}};
#endif

        if (DEBUG_LOG)
            std::cout << "starting write of " << len << " bytes\n";
    ...
    void socket_impl::close()
    {
        if (!closed_)
        {
            ...
            {
                if (DEBUG_LOG)
                    std::cout << "Closing disconnected socket\n";
            }
            close_handle(&tcp_);
            cond_.notify_all();
        }
