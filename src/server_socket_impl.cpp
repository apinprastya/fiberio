...
server_socket_impl::server_socket_impl() : loop_{get_uv_loop()}, pending_connections_{0}, closed_{false} {
    if (DEBUG_LOG)
        std::cout << "creating server_socket_impl\n";
...
server_socket_impl::~server_socket_impl() {
    if (!closed_) {
        if (DEBUG_LOG)
            std::cout << "closing server_socket_impl from destructor\n";
        close();
    } else {
        if (DEBUG_LOG)
            std::cout << "destroying closed server_socket_impl\n";
    }
}

void server_socket_impl::bind(const std::string &host, uint16_t port) {
    if (DEBUG_LOG)
        std::cout << "calling getaddrinfo for " << host << ":" << port << "\n";
    host_ = host;
    port_ = port;
...
std::string server_socket_impl::get_host() { return host_; }

uint16_t server_socket_impl::get_port() { return port_; }
...
void server_socket_impl::on_connection(int status) {
    try {
        ...
        if (DEBUG_LOG)
            std::cout << "increased pending_connections_ to " << pending_connections_ << "\n";
        cond_.notify_all();
    } catch (std::exception &e) {
        if (DEBUG_LOG)
            std::cout << "on_connection error: " << e.what() << "\n";
        close();
    }
}
...
socket server_socket_impl::accept() {
    ...
    if (DEBUG_LOG)
        std::cout << "waiting for connection to accept\n";
    dummy_lock lock;
    ...
    // Accept connection
    if (DEBUG_LOG)
        std::cout << "going to accept pending connection\n";
    pending_connections_--;
...
void server_socket_impl::close() {
    if (!closed_) {
        if (DEBUG_LOG)
            std::cout << "closing server_socket_impl\n";
        closed_ = true;
        cond_.notify_all();
        close_handle(&tcp_);
    }
