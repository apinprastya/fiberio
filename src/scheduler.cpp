...
namespace fiberio
{
    ...
    namespace
    {
        ...
        {
            if (DEBUG_LOG)
                std::cout << "suspend_timer_callback\n";
    ...
    {
        if (DEBUG_LOG)
            std::cout << "scheduler: creating scheduler\n";
        loop_ = get_uv_loop();
    ...
    {
        if (DEBUG_LOG)
            std::cout << "scheduler: destroying scheduler\n";
    }

    void scheduler::awakened(fibers::context *fiber) noexcept
    {
        if (DEBUG_LOG)
            std::cout << "scheduler::awakened(" << fiber->get_id() << ")\n";
    ...
    fibers::context *scheduler::pick_next() noexcept
    {
        ...
        {
            if (DEBUG_LOG)
                std::cout << "scheduler::pick_next() -> (no fiber)\n";
            return nullptr;
        }
        else
        {
            ...
            queue_.pop_front();
            if (DEBUG_LOG)
                std::cout << "scheduler::pick_next() -> " << fiber->get_id() << "\n";
            return fiber;
        }
    }
    ...
    {
        if (DEBUG_LOG)
            std::cout << "scheduler::suspend_until()\n";

        const bool wake_up_directly{suspend_enter()};
        if (wake_up_directly)
        {
            if (DEBUG_LOG)
                std::cout << "not running event loop\n";
            suspend_exit();
            return;
        }
        ...
        else
        {
            ...
            if (DEBUG_LOG)
                std::cout << "setting timer for " << milliseconds << " ms\n";
            timer_set = true;
        ...
        while (true)
        {
            ...
            {
                if (DEBUG_LOG)
                    std::cout << "exiting from suspend_until()\n";
                break;
            }
        }
    ...
    {
        if (DEBUG_LOG)
            std::cout << "scheduler::notify()\n";
        bool suspended = update_notify();
        if (suspended)
        {
            if (DEBUG_LOG)
                std::cout << "scheduler uv_async_send()\n";
            uv_async_send(async_);
        }
        else
        {
            if (DEBUG_LOG)
                std::cout << "scheduler not suspended\n";
        }
    }
